...
static bsec_library_return_t bme68x_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_ready_fct output_ready)
{
    ...
    uint8_t breath_voc_equivalent = 0;
    ...
    if (num_bsec_inputs > 0)
    {
        ...
        for (index = 0; index < num_bsec_outputs; index++)
        {
            ...
            switch (bsec_outputs[index].sensor_id)
            {
                ...
                case BSEC_OUTPUT_BREATH_VOC_EQUIVALENT:
                    breath_voc_equivalent = (uint8_t )bsec_outputs[index].signal;
        ...
        output_ready(timestamp, iaq, iaq_accuracy, temp, humidity, raw_pressure, raw_temp,
                     static_iaq, co2_equivalent, breath_voc_equivalent, bsec_status);
    }
	return bsec_status;
}
    ...
    /*while (1)*/
    {
		...
		if (time_stamp >= sensor_settings.next_call)
		{
			...
			if (bsec_status != BSEC_OK)
			{
				if (bsec_status < BSEC_OK)
				{
					TRACE_INFO("ERROR: bsec_sensor_control: %d\r\n", bsec_status);
					break;
				}
				else
				{
					TRACE_INFO("WARNING: bsec_sensor_control: %d\r\n", bsec_status);
				}
			}
